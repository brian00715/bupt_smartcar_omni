...
void Inducer_Show_Oled(void)
{
    ...
    Inducer_Get[0]=adc_mean_filter(ADC_IN0_A0,ADC_12BIT,10);        //12BIT分辨率,取五次平均
    ...
    //Inducer_Get[5]=adc_mean_filter(ADC_IN5_A5,ADC_12BIT,5);        //12BIT分辨率,取五次平均
    //Inducer_Get[6]=adc_mean_filter(ADC_IN6_A6,ADC_12BIT,5);        //12BIT分辨率,取五次平均
...
void InducerMax_Get(void)
{
    if(InducerMax_Get_Start_Flag==1)        //如果电感获取值标志为1则获取电感最大值
    {
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[1]=adc_mean_filter(ADC_IN4_A4,ADC_12BIT, 10);          //12BIT分辨率,取五次平均
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[2]=adc_mean_filter(ADC_IN6_A6,ADC_12BIT, 10);          //12BIT分辨率,取五次平均
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[3]=adc_mean_filter(ADC_IN8_B0,ADC_12BIT, 10);          //12BIT分辨率,取五次平均
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[4]=adc_mean_filter(ADC_IN9_B1,ADC_12BIT, 10);          //12BIT分辨率,取五次平均
...
void Inducer_Processing(void)
{
    ...
    if(InducerMax_Get_End_Flag==1)                   //电感获取最大值标志为1时才进行数据处理，避免有最大值为0、后续除法出错的危险
    {
        ...
        Inducer_Get[0]=adc_mean_filter(ADC_IN0_A0, ADC_12BIT, 10);        //12BIT精度，取5次AD转换平均值
        Inducer_Get[1]=adc_mean_filter(ADC_IN4_A4, ADC_12BIT, 10);        //12BIT精度，取5次AD转换平均值
        Inducer_Get[2]=adc_mean_filter(ADC_IN6_A6, ADC_12BIT, 10);        //12BIT精度，取5次AD转换平均值
        Inducer_Get[3]=adc_mean_filter(ADC_IN8_B0, ADC_12BIT, 10);        //12BIT精度，取5次AD转换平均值
        Inducer_Get[4]=adc_mean_filter(ADC_IN9_B1, ADC_12BIT, 10);        //12BIT精度，取5次AD转换平均值
        //Inducer_Get[5]=adc_mean_filter(ADC_IN5_A5, ADC_12BIT, 5);        //12BIT精度，取5次AD转换平均值
        //Inducer_Get[6]=adc_mean_filter(ADC_IN6_A6, ADC_12BIT, 5);        //12BIT精度，取5次AD转换平均值
