...
uint16 InducerMax[7]={0,0,0,0,0,0,0};      //用于存放电感最大值以进行归一化处理
...
static int i=0,j=0;    //循环次数辅助

/************************************************************************************
  * @brief        将4个电感值在oled上实时显示
  * @param        NULL
  * @return       void
  * Sample usage:
  **********************************************************************************/
...
void InducerMax_Get(void)
{
    if(InducerMax_Get_Start_Flag==1)        //如果电感获取值标志为1则获取电感最大值
    {
        ...
        {
            Inducer_Get[0]=adc_mean_filter(ADC_IN4_A4,ADC_12BIT, 10);          //12BIT分辨率,取10次平均
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[1]=adc_mean_filter(ADC_IN6_A6,ADC_12BIT, 10);          //12BIT分辨率,取10次平均
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[2]=adc_mean_filter(ADC_IN8_B0,ADC_12BIT, 10);          //12BIT分辨率,取10次平均
        ...
        for(j=0;j<Inducer_Get_Count;j++)      //每个电感采集数据数量
        {
            Inducer_Get[3]=adc_mean_filter(ADC_IN9_B1,ADC_12BIT, 10);          //12BIT分辨率,取10次平均
        ...
        /****************电感最大值获取结束**************************************************/
        //systick_delay_ms(2000);                  //延时3秒，方便通过oled读数
...
void Inducer_Processing(void)
{
    ...
    if(InducerMax_Get_End_Flag==1)                   //电感获取最大值标志为1时才进行数据处理，避免有最大值为0、后续除法出错的危险
    {
        ...
        Inducer_Get[0]=adc_mean_filter(ADC_IN4_A4, ADC_12BIT, 10);        //12BIT精度，取10次AD转换平均值
        Inducer_Get[1]=adc_mean_filter(ADC_IN6_A6, ADC_12BIT, 10);        //12BIT精度，取10次AD转换平均值
        Inducer_Get[2]=adc_mean_filter(ADC_IN8_B0, ADC_12BIT, 10);        //12BIT精度，取10次AD转换平均值
        Inducer_Get[3]=adc_mean_filter(ADC_IN9_B1, ADC_12BIT, 10);        //12BIT精度，取10次AD转换平均值
