...
//  @return     void
//  @since      v1.0
//  Sample usage:      Uart_Sendimg(UART_1,camera_buffer_addr,MT9V03X_W, MT9V03X_H);//UART1发送摄像头数据
//-------------------------------------------------------------------------------------------------------------------
...
void Uart_Sendimg(UARTN_enum uartn, uint8 *image, uint16 width, uint16 height)      //山外调试助手图像发送格式
{
    uart_putchar(uartn,0x01);uart_putchar(uartn,0xFE);
    uart_putbuff(uartn, image, width*height);
    uart_putchar(uartn,0xFE);uart_putchar(uartn,0x01);
}
